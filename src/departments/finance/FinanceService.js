...
class FinanceService {
  ...
  async getFinancialData(filters = {}) {
    ...
    try {
      ...
      console.warn('⚠️ Using dummy financial data - API not available:', error.message);
  ...
  async getGLAccountNames() {
    ...
    try {
      ...
      console.warn('⚠️ Using dummy account names - API not available:', error.message);
  ...
  async getDocumentTypes() {
    ...
    try {
      ...
      const documentTypes = Array.from(typesSet).sort().map(type => ({
        value: type,
        label: type,
        displayName: type
      }));
      ...
      console.warn('⚠️ Using dummy document types - API not available:', error.message);
  ...
  calculateCloseMetrics(data, filters = {}) {
    ...
    const grouped = groupBy(data, entry => {
      ...
      switch (granularity) {
        ...
        case 'monthly':
        default:
          return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; // e.g., 2021-08
      }
    });
    ...
    const metrics = Object.entries(grouped).map(([period, entries]) => {
      ...
      return {
        ...
        debitCreditRatio: totalCredits > 0 ? parseFloat((totalDebits / totalCredits).toFixed(2)) : null,
  ...
  sumDebits(transactions) {
    return transactions.reduce((sum, t) => sum + parseFloat(t.debitAmount || 0), 0);
  }

  sumCredits(transactions) {
    return transactions.reduce((sum, t) => sum + parseFloat(t.creditAmount || 0), 0);
  }
  ...
  getDummyAccountNames() {
    return [
      { accountNo: "10060", accountName: "Mbarara Cash Control", displayName: "10060 - Mbarara Cash Control" },
      { accountNo: "21001", accountName: "Accounts Payable", displayName: "21001 - Accounts Payable" },
      { accountNo: "40000", accountName: "Sales Revenue", displayName: "40000 - Sales Revenue" },
      { accountNo: "50000", accountName: "Cost of Goods Sold", displayName: "50000 - Cost of Goods Sold" },
      { accountNo: "60000", accountName: "Operating Expenses", displayName: "60000 - Operating Expenses" }
    ];
  }
  ...
  getDummyDocumentTypes() {
    return [
      ...
      { value: "Credit Memo", label: "Credit Memo", displayName: "Credit Memo" },
