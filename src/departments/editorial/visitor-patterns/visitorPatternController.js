...
export const getVisitorAnalytics = (req, res) => {
  ...
  // --- Calculate and attach wait time for each visitor ---
  ...
  // --- Identify peak visiting hours (filtered data only) ---
  const visitsByHour = {};
  filtered.forEach(v => {
  ...
  // --- Identify peak days of the week (filtered data only) ---
  const visitsByDay = {};
  ...
  // --- Categorize and count visits by department and Visit Type (filtered data only) ---
  ...
  // --- Calculate average wait time per day, hour, and month (filtered data only) ---
  const waitTimeByDay = {};
  ...
  // --- Calculate percentage of walkouts or excessive wait times ---
