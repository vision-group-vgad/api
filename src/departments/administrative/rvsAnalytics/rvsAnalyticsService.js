...
class rvsAnalyticsService {
  ...
  async fetchAllResources({ startDate, endDate, department, resourceType, page = 1, pageSize = 10 }) {
    this.initialize();
    console.log("🔍 [RVS Service] fetchAllResources called", { startDate, endDate, department, resourceType, page, pageSize });
    
    const cacheKey = `resources_${startDate}_${endDate}_${department || "all"}_${resourceType || "all"}_${page}_${pageSize}`;
    if (cache.has(cacheKey)) {
      const { data, totalCount, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) {
        console.log("💾 [RVS Service] Returning cached resources:", data.length);
        return { resources: data, totalCount };
      }
    }

    // For now, use dummy data. When API is available, replace with:
    // const endpoint = `/api-listings/resources/${startDate}/${endDate}/${(page-1)*pageSize}`;
    ...
    cache.set(cacheKey, { data: paginatedResources, totalCount, timestamp: Date.now() });
    console.log(`✅ [RVS Service] Fetched resources: ${paginatedResources.length} (page ${page}, totalCount: ${totalCount})`);
  ...
  async fetchAllSpaces({ startDate, endDate, location, department, page = 1, pageSize = 10 }) {
    this.initialize();
    console.log("🔍 [RVS Service] fetchAllSpaces called", { startDate, endDate, location, department, page, pageSize });
    
    const cacheKey = `spaces_${startDate}_${endDate}_${location || "all"}_${department || "all"}_${page}_${pageSize}`;
    if (cache.has(cacheKey)) {
      const { data, totalCount, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) {
        console.log("💾 [RVS Service] Returning cached spaces:", data.length);
        return { spaces: data, totalCount };
      }
    }
    ...
    cache.set(cacheKey, { data: paginatedSpaces, totalCount, timestamp: Date.now() });
    console.log(`✅ [RVS Service] Fetched spaces: ${paginatedSpaces.length} (page ${page}, totalCount: ${totalCount})`);
  ...
  async fetchAllVendors({ startDate, endDate, serviceType, vendor, page = 1, pageSize = 10 }) {
    this.initialize();
    console.log("🔍 [RVS Service] fetchAllVendors called", { startDate, endDate, serviceType, vendor, page, pageSize });
    
    const cacheKey = `vendors_${startDate}_${endDate}_${serviceType || "all"}_${vendor || "all"}_${page}_${pageSize}`;
    if (cache.has(cacheKey)) {
      const { data, totalCount, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) {
        console.log("💾 [RVS Service] Returning cached vendors:", data.length);
        return { vendors: data, totalCount };
      }
    }
    ...
    cache.set(cacheKey, { data: paginatedVendors, totalCount, timestamp: Date.now() });
    console.log(`✅ [RVS Service] Fetched vendors: ${paginatedVendors.length} (page ${page}, totalCount: ${totalCount})`);
  ...
  async getResourceKPIs(filters) {
    console.log("📊 [RVS Service] getResourceKPIs called", filters);
  ...
  async getSpaceKPIs(filters) {
    console.log("📊 [RVS Service] getSpaceKPIs called", filters);
  ...
  async getVendorKPIs(filters) {
    console.log("📊 [RVS Service] getVendorKPIs called", filters);
    ...
    return {
      totalVendors,
      activeVendors: totalVendors, // All vendors in dummy data are active
  ...
  async getResourceChartData({ metric, groupBy, ...filters }) {
    console.log("📈 [RVS Service] getResourceChartData called", { metric, groupBy, ...filters });
    const { resources } = await this.fetchAllResources({ ...filters, page: 1, pageSize: 1000 });
    ...
    console.log(`📈 [RVS Service] Resource chart data generated: ${chartData.length} groups`);
    return chartData;
  }

  async getSpaceChartData({ metric, groupBy, ...filters }) {
    console.log("📈 [RVS Service] getSpaceChartData called", { metric, groupBy, ...filters });
    const { spaces } = await this.fetchAllSpaces({ ...filters, page: 1, pageSize: 1000 });
    ...
    console.log(`📈 [RVS Service] Space chart data generated: ${chartData.length} groups`);
    return chartData;
  }

  async getVendorChartData({ metric, groupBy, ...filters }) {
    console.log("📈 [RVS Service] getVendorChartData called", { metric, groupBy, ...filters });
    const { vendors } = await this.fetchAllVendors({ ...filters, page: 1, pageSize: 1000 });
    ...
    console.log(`📈 [RVS Service] Vendor chart data generated: ${chartData.length} groups`);
    return chartData;
  }
  ...
  async getResourceTypes() {
    this.initialize();
    const cacheKey = "all_resource_types";
    if (cache.has(cacheKey)) {
      const { data, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) return data;
    }
  ...
  async getLocations() {
    ...
    const locations = [...new Set(spaces.map(s => s.location))];
  ...
  async getServiceTypes() {
    this.initialize();
    const cacheKey = "all_service_types";
    if (cache.has(cacheKey)) {
      const { data, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) return data;
    }
    ...
    const types = [...new Set(vendors.map(v => v.service_type))];
    cache.set(cacheKey, { data: types, timestamp: Date.now() });
    return types;
  }
  ...
  async getVendorNames() {
    this.initialize();
    const cacheKey = "all_vendor_names";
    if (cache.has(cacheKey)) {
      const { data, timestamp } = cache.get(cacheKey);
      if (Date.now() - timestamp < CACHE_TTL) return data;
    }
    ...
    const vendors = generateVendorData();
    const names = [...new Set(vendors.map(v => v.vendor_name))];
